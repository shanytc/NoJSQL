...
var nojsql = {
	...
	parseSQL : function(){
		...
		results = this.getDataFromDB(select,from,where,orderby,limit);
		// return data
		return results; // remove this line
	},
	getDataFromDB: function(select,from,where,orderby,limit){ // main function that strips all the info from the db
		params = [];
		if(!this.getDB()[from].length){
			this.errors.push('Table \''+from+'\' has no data in database!.');
			console.error('Table \''+from+'\' has no data in database!.');
	...
	getWhere: function(){
		...
		// test for the existence of a 'where' clause
		var regex=/where\s.*/g;
		if(!regex.test(sql)){
			this.errors.push('invalid or missing \'where\' statement in selected query');
			console.error('invalid or missing \'where\' statement in selected query');
			return [];
		}

		// test for where clause fields
		var where = this.trim(sql.match(regex)[0].replace(/where\s/,''));

		// check the where logic here
		where_logic = [];
		logic_counter=0;
		logic = where.split(' ');
		for(i=0; i<logic.length;i++){
			s = this.trim(logic[i]);
			if(s=='and'){
				where_logic[logic_counter]='and';
				logic_counter++;
			}
			if(s=='between'){
				where_logic[logic_counter]='between';
				logic_counter++;
			}
			if(s=='or'){
				where_logic[logic_counter]='or';
				logic_counter++;
			}
		}
		console.log(where_logic);

		where = where.replace(/and\s/,'$seperetor$ ');
		where = where.replace(/or\s/,'$seperetor$ ');
		
		where = where.split('$seperetor$');
		...
		for(var i = 0; i < where.length; i++) { // loop where commands
			w = this.trim(where[i]);
		...
		if(where_commands.length)
			return [where_commands,where_logic];
	},
	getLimit: function(){
		sql = this.getSQL();
		var regex=/limit\s([0-9]+\;|[0-9]+,[0-9]+\;)/g;
